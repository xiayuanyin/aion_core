...
#[tokio::test]
async fn hydrate_defers_external_acp_installation_check() {
    let db = init_database_memory().await.unwrap();
    let repo: Arc<dyn IAgentMetadataRepository> = Arc::new(SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    assert!(cached.installed, "startup hydrate should defer external ACP checks");
    assert_eq!(cached.last_check_status, Some(AgentSnapshotCheckStatus::Online));
}

#[tokio::test]
async fn hydrate_defers_commandless_managed_builtin_installation_check() {
    let db = init_database_memory().await.unwrap();
    let repo: Arc<dyn IAgentMetadataRepository> = Arc::new(SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    assert!(
        cached.installed,
        "startup hydrate should defer managed builtin installation checks"
    );
    assert_eq!(cached.last_check_status, Some(AgentSnapshotCheckStatus::Online));
}
